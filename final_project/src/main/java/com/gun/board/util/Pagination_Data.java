...
public class Pagination_Data {
	...
	// 총 페이지 계산
	public int totalPages(ArrayList<Data> boards_data) {
		int result = 0;
		result = (boards_data.size() / Configuration.POSTS);
		if (boards_data.size() % Configuration.POSTS != 0 || result == 0) {
			result += 1;
		}
		return result;
	}
	...
	public ArrayList<Data> totalPosts_home_data(ArrayList<Data> data, int page) {
        ArrayList<Data> result = new ArrayList();
